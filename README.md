...
Listener по умолчанию ищет `cinnamon-xkb-switch` рядом с собой в том же каталоге. При необходимости путь можно переопределить переменной `LAYOUT_SWITCH_CMD`.
