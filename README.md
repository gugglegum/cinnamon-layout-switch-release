...
На Linux Mint 22.3 / Cinnamon 6.6.x всплывает и дополнительная проблема: старые решения, которые дёргают XKB напрямую через `xkb-switch -s ...` или через `ibus engine ...`, могут рассинхронизировать реальное состояние раскладки и апплет Cinnamon в панели. Внешне это выглядит так:
...
## Почему используется не `xkb-switch -s`, а D-Bus Cinnamon
...
## Идея проекта в одной фразе

Не пытаться насильно переключать XKB за спиной у Cinnamon, а дать Cinnamon самому переключить раскладку в правильный момент — на отпускании модификаторов.
